...
export class DocumentProcessingService {
  ...
  private async extractDataWithAI(text: string): Promise<ExtractedDataDto> {
    ...
    const systemPrompt = `
      ...
      - completedDate: string | null (Date in YYYY-MM-DD format, null if not found.  There frequently is a date in the text, use that if so.)
      ...
      - expirationDate: string | null (Date in YYYY-MM-DD format, null if not found)
